...
var App = function() {
    ...
    var uiInit = function() {
        ...
        // Autogrow
        jQuery('.js-autogrow').each(function(){
            jQuery(this).autogrow();
        });

        // DataTables
        jQuery('.js-datatables').each(function(){
            jQuery(this).DataTable({ responsive: true });
        });

        // Datepicker
        jQuery('.js-date').each(function(){
            jQuery(this).datepicker({ format: 'dd/mm/yyyy'});
        });

        // Autocomplete Users        
        jQuery('.js-autocomplete-users').each(function(){
            var fxhr;
            $('.js-autocomplete-users').autoComplete({
                source: function(term, response){
                    term = term.toLowerCase();
                    try { fxhr.abort(); } catch(e){}
                    fxhr = $.ajax({
                        method: "POST",
                        url: "../../../../ajax.php",
                        dataType: "json",
                        data: { op: "get_users_json", q: term }
                ...
                renderItem: function(item, search) {
                    ...
                    return '<div class="autocomplete-suggestion" data-fid="'+item[0]+'" data-val="' + item[1] + '">'+ item[1].replace(re, "<b>$1</b>") + '</div>';
                },
                onSelect: function(e, term, item){                    
                    $('#pm_to').val(item.data('fid'));
                }
            });
        });

        // Emojis
        var emojiStrategy = <?php echo read_file(WEB_VENDOR.'emojione/emojione/emoji_strategy.json');?>;
        jQuery('.js-emojis').each(function(){
            jQuery(this).textcomplete([{
                ...
                footer: '<a href="http://www.emoji.codes" target="_blank"><?php echo _BROWSE_ALL;?><span class="arrow">»</span></a>'
            });
        });
    };
            ...
            if ($elBlock.length) {
                ...
                switch($mode) {
                    ...
                    case 'refresh_toggle':
                        ...
                        // Messages
                        if (jQuery('[data-toggle="block-option"][data-action="refresh_toggle"][data-action-mode="refresh-inbox"]', $elBlock).length) {
                            iLoaderStatic('messages','load-msg','');
                        }

                        // Refresh Notifications
                        if (jQuery('[data-toggle="block-option"][data-action="refresh_toggle"][data-action-mode="refresh-notifications"]', $elBlock).length) {
                            setTimeout(function(){
                        ...
                        // Refresh People Online
                        if (jQuery('[data-toggle="block-option"][data-action="refresh_toggle"][data-action-mode="refresh-people-online"]', $elBlock).length) {
                            setTimeout(function(){
                                getData('online');
                                $elBlock.removeClass('block-opt-refresh');
                            }, 1000);
    ...
    **************************************************************/
    var initLoginValidation = function(){
        jQuery('.js-login-validation').validate({
            ignore: [],
            ...
            rules: {
                ...
                <?php if ( ($config->SecurityCheck==true) && (strlen($config->GoogleCaptchaKey)==0) ) { ?>
                ,
                '_sec': {
                    required: true
                }
            ...
            messages: {
                'loginid': {
                    <?php if ($config->LoginID == 'EMAIL') { ?>
                    required: '<?php echo _PLEASE_ENTER_YOUR_EMAIL; ?>'
                    <?php } else { ?>
                    required: '<?php echo _PLEASE_ENTER_YOUR_LOGINID; ?>'
                    <?php } ?>
                },
                'pwd': {
                    required: '<?php echo _PLEASE_ENTER_YOUR_PASSWORD; ?>'
                }
                <?php if ( ($config->SecurityCheck==true) && (strlen($config->GoogleCaptchaKey)==0) ) { ?>
                ,
                '_sec': {
                    required: '<?php echo _PLEASE_ENTER_SECURITY_CODE; ?>',
                }
                <?php } ?>
            }
        });
    ...
    var initRegisterValidation = function(){
        jQuery('.js-register-validation').validate({
            ignore: [],
            errorClass: 'help-block text-right animated fadeInDown',
            errorElement: 'div',
            errorPlacement: function(error, e) {
            ...
            messages: {
                ...
                '_firstname': {
                    required: '<?php echo _PLEASE_ENTER_YOUR_FIRSTNAME; ?>'
                },
                '_lastname': {
                    required: '<?php echo _PLEASE_ENTER_YOUR_LASTNAME; ?>'
                },
                '_email': {
                    required: '<?php echo _PLEASE_ENTER_YOUR_EMAIL; ?>',
                    email: '<?php echo _PLEASE_ENTER_YOUR_VALID_EMAIL; ?>'
                }
                <?php if ( ($config->SecurityCheck==true) && (strlen($config->GoogleCaptchaKey)==0) ) { ?>
                ,
                '_sec_register': {
    ...
    var initPMValidation = function(){
        jQuery('.js-pm').validate({
            ignore: [],
            errorClass: 'help-block text-right animated fadeInDown',
            errorElement: 'div',
            errorPlacement: function(error, e) {
            ...
            messages: {
                '_to': {
                    required: '<?php echo _PLEASE_SELECT_RECIPIENT; ?>'
                },
                'pm_txt': {
                    required: '<?php echo _PLEASE_ENTER_YOUR_MESSAGE; ?>',
                    minlength: '<?php echo sprintf(_PLEASE_ENTER_AT_LEAST, 10); ?>'
                }
            }
        });
    };
    ...
    var uiHelperMagnific = function(){
        // Single View
        jQuery('.js-gallery-single').each(function(){
    ...
    var uiHelperSummernote = function(){
        ...
        // Init full text editor
        jQuery('.js-summernote-min').summernote({
            height: 150,
            minHeight: null,
            maxHeight: null
        });
    ...
    var uiHelperTagsInputs = function(){
        ...
        jQuery('.js-tags-input').each(function(){
    ...
    var uiHelperSelect2 = function(){
        ...
        jQuery('.js-select2-nosearch').select2({minimumResultsForSearch: Infinity});
    };
    ...
    var uiHelperDraggableItems = function(){
        ...
        jQuery('.js-draggable-items > .draggable-column').sortable({
            ...
            update: function(e, ui){
                var data_sort = $(this).sortable('serialize');
                $.ajax({
                    method: "POST",
                    url: "../../../../ajax.php",
                    data: {
    ...
    /*
     * Auto Load Data - Static Data
     */
    var iLoaderStatic = function($what,$element,$id) {
        var divimg = '<div id="'+$element+'-loading" class="text-center push-10 push-10-t"><i class="fa fa-cog fa-spin"></i></div>';
    ...
    return {
        ...
        initLoaderStatic: function($what,$element,$id) {
            iLoaderStatic($what,$element,$id);
        },
        gData: function($what,$id) {
            getData($what,$id);
        }
    };
}();
